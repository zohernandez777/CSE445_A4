            ...
            try
            {
                ...
                using (var webClient = new System.Net.WebClient()) //webclient to download the xml
                {
                    ...
                    foreach (XmlNode hotelNode in hotels)
                    {
                        ...
                        XmlNodeList phoneNodes = hotelNode.SelectNodes("Phone"); //select all <Phone> nodes
                        foreach (XmlNode phoneNode in phoneNodes) //loop through each phone node
                            phoneList.Add(phoneNode.InnerText.Trim()); //add the text from the node to the list
